...
- **Raw data:** `reddit_depression_dataset.zip` in Project root
- **Prepared splits:** `train.csv`, `val.csv`, `test.csv`, `small_dev.csv` — all in Google Drive root, all generated by `NLPDataSetup_Baseline.ipynb`
- All teammates should load from these saved CSVs directly rather than re-running the preparation notebook, to guarantee everyone is working from the exact same dataset.

---

## How to Run the Code

All notebooks run in Google Colab. Open in Colab, mount your Drive when prompted, and run cells top to bottom.

**Order matters:**

1. **`NLPDataSetup_Baseline.ipynb`** — run this first. It generates the four CSV splits that every other notebook depends on. Do not re-run this once the splits are saved, or you risk regenerating them with different random seeds.
